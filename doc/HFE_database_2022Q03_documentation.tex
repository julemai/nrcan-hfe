...
\section{Prepare, read, and check data from HFE database} 
...
\textbf{Step B2:} Read data from HFE files.
\begin{framed}
	\vspace*{-1.2cm}
	\begin{ttfont}
		\begin{tabbing}
			...
			\> \> \GRAU{\# to be consistent}\\
			\> \> \GRAU{\# (checks applied are listed below)}\\
...
Table~\ref{tab:HFE_JSON_feautues} shows the number of features available in the files provided. Different levels of checks are applied, i.e., no checking, checking everything but not location of coordinates within a polygon (check \#1 to \#3 but not \#4) and testing also the location of the coordinates of the features (checks \#1 to \#4). The polygon is set to the extend used to request the CaSPAr data (see \texttt{data/caspar/RDRS\_v21\_hfe\_outline.zip}).
